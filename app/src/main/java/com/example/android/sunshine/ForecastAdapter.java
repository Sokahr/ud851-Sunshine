...
//      DONE (7) Generate a weather summary with the date, description, high and low
        long normalizedUtcMidnight = mCursor.getLong(MainActivity.INDEX_COLUMN_DATE);
        String dateString = SunshineDateUtils.getFriendlyDateString(mContext, normalizedUtcMidnight, false);
        int weatherId = mCursor.getInt(MainActivity.INDEX_COLUMN_WEATHER_ID);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);
        double highTemperature = mCursor.getDouble(MainActivity.INDEX_COLUMN_HIGH_TEMPERATURE);
        double lowTemperature = mCursor.getDouble(MainActivity.INDEX_COLUMN_LOW_TEMPERATURE);
        String highAndLowTemperature = SunshineWeatherUtils.formatHighLows(mContext, highTemperature, lowTemperature);
